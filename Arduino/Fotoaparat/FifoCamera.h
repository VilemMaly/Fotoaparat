...
class FifoCamera
{
  ...
  static const int REG_REG74 = 0x74;
  static const int REG_REG75 = 0x75;
  static const int REG_LCC1 = 0x62;
  static const int REG_LCC2 = 0x63;
  static const int REG_LCC4 = 0x65;
  static const int REG_LCC3 = 0x64;
  static const int REG_LCC5 = 0x66;
  static const int REG_LCC6 = 0x94;
  static const int REG_LCC7 = 0x95;
...
void BinToSD(
    ...
    sd.BMPHeaderRGB565(XRES, YRES);

    // synchronizace snímku
    while (!digitalRead(VSYNCCAMERA));
    while ( digitalRead(VSYNCCAMERA));
    prepareCapture();
    startCapture();
    while (!digitalRead(VSYNCCAMERA));
...
void FrameToDisplay(TFT_eSPI& tft, int XRES, int YRES) {
    fifo.readReset();
    
    // STM32F103C8 (Bluepill) má 20KB RAM
    // Po odečtení stacku, globálních proměnných atd. můžeme využít ~15-16KB
    // Jeden pixel = 2 bajty, tedy můžeme uložit ~7500-8000 pixelů
    
    // Pro QVGA (320×240): celý frame = 153,600 bajtů (příliš mnoho)
    // Můžeme uložit ~20 řádků (320px × 20 = 19,200 pixelů = 12 800 bajtů)
    
    #define BUFFER_LINES 20
    uint16_t frameBuffer[BUFFER_LINES * XRES];
    
    tft.startWrite();
    
    // Počet celých bloků
    int fullBlocks = YRES / BUFFER_LINES;
    int remainingLines = YRES % BUFFER_LINES;
    
    // Zpracuj celé bloky
    for (int block = 0; block < fullBlocks; block++) {
        int startY = block * BUFFER_LINES;
        ...
        // Odešli celý blok najednou
        tft.setAddrWindow(0, startY, XRES, BUFFER_LINES);
        tft.pushPixels(frameBuffer, BUFFER_LINES * XRES);
    }
    
    // Zpracuj zbývající řádky (pokud existují)
    if (remainingLines > 0) {
        int startY = fullBlocks * BUFFER_LINES;
        ...
        // Odešli zbývající řádky
        tft.setAddrWindow(0, startY, XRES, remainingLines);
        tft.pushPixels(frameBuffer, remainingLines * XRES);
    }
    
    tft.endWrite();
}

void testFifo(int XRES, int YRES)
{
  ...
  void QVGA()
  {
    ...
    i2c.writeRegister(ADDR, REG_SCALING_XSC, 0b00111010);
    i2c.writeRegister(ADDR, REG_SCALING_YSC, 0b00110101);
    i2c.writeRegister(ADDR, REG_SCALING_DCWCTR, 0x11); //downsample by 2
    i2c.writeRegister(ADDR, REG_SCALING_PCLK_DIV, 0b1111001); //pixel clock divided by 2
  ...
  void fps()
  {
    ...
    i2c.writeRegister(ADDR, 0x92, 0x00);
    i2c.writeRegister(ADDR, 0x93, 0x00);
    i2c.writeRegister(ADDR, 0x3b, 0x0a);
  ...
  void VGA()
{
  ...
  // - DCW OFF
  // - PCLK = 1:1
  i2c.writeRegister(ADDR, REG_COM14, 0x00);

  // DCWCTR:
  // 0000 0000
  // - no downsampling
  i2c.writeRegister(ADDR, SCALING_DCWCTR, 0x00);

  // PCLK_DIV:
  // 0000 0000
  // - no division
  i2c.writeRegister(ADDR, SCALING_PCLK_DIV, 0x00);
  fps();
}

  void VGARGB565()
{
  i2c.writeRegister(ADDR, REG_COM7, 0b10000000);  // reset všech registrů
        
  i2c.writeRegister(ADDR, REG_CLKRC, 0b10000000); // double clock (XCLK / 2)
  i2c.writeRegister(ADDR, REG_COM11, 0b1000 | 0b10); // auto 50/60Hz + kratší expozice
  i2c.writeRegister(ADDR, REG_TSLB, 0b100); // UYVY sekvence

  i2c.writeRegister(ADDR, REG_COM7, 0b100); // RGB mode
  i2c.writeRegister(ADDR, REG_COM15, 0b11000000 | 0b010000); // RGB565

  // VGA frame window – typické hodnoty
  frameControl(
    158, 14,   // HSTART, HSTOP
    10, 490    // VSTART, VSTOP
  );
  ...
  i2c.writeRegister(ADDR, REG_COM3, 0x00);  // DCW OFF
  i2c.writeRegister(ADDR, REG_MVFP, 0x0B);  // mirror/flip (stejné jako QQVGA)

  VGA();

  i2c.writeRegister(ADDR, 0xB0, 0x84); // color magic register
  saturation(0);
...
void VGAYUV()
{
  ...
  i2c.writeRegister(ADDR, REG_COM10, 0x02); // VSYNC negative
  i2c.writeRegister(ADDR, REG_COM3, 0x00);  // DCW OFF
  i2c.writeRegister(ADDR, REG_MVFP, 0x2B);  // mirror + flip (stejné chování)

  // rozlišení
  VGA();

  // color pipeline
  i2c.writeRegister(ADDR, 0xB0, 0x84); // interní color matrix / gamma
  i2c.writeRegister(ADDR, 0x13, 0xE7); // AWB ON
  i2c.writeRegister(ADDR, 0x6F, 0x9F); // Simple AWB
}
  ...
  void QVGARGB565()
  {
    ...
    i2c.writeRegister(ADDR, REG_COM10, 0x02); //VSYNC negative
    i2c.writeRegister(ADDR, REG_MVFP, 0b00001011);  //mirror flip
    ...
    saturation(0);
    i2c.writeRegister(ADDR, 0x13, 0xe7); //AWB on
    i2c.writeRegister(ADDR, 0x6f, 10011011); // Simple AWB
  }
  ...
  void QQVGARGB565()
  {
    ...
    i2c.writeRegister(ADDR, REG_COM10, 0x02); //VSYNC negative
    i2c.writeRegister(ADDR, REG_COM3, 0x04);  //DCW enable
    i2c.writeRegister(ADDR, REG_MVFP, 0xB);  //mirror flip
  ...
  private:
    void writeUint16(SdFile &f, uint16_t value) {
        f.write(value & 0xFF);
        f.write((value >> 8) & 0xFF);
    }
    
    void writeUint32(SdFile &f, uint32_t value) {
        f.write(value & 0xFF);
        f.write((value >> 8) & 0xFF);
    ...
    void writeBMPHeader(SdFile &f, int width, int height) {
        uint32_t fileSize = 54 + (width * height * 3);
        uint32_t imageSize = width * height * 3;
        
        f.write('B');
        f.write('M');
        writeUint32(f, fileSize);
        writeUint32(f, 0);
        writeUint32(f, 54);
        
        writeUint32(f, 40);
        writeUint32(f, width);
        writeUint32(f, height);
        writeUint16(f, 1);
        writeUint16(f, 24);
        writeUint32(f, 0);
        writeUint32(f, imageSize);
        writeUint32(f, 2835);
        writeUint32(f, 2835);
        writeUint32(f, 0);
        writeUint32(f, 0);
